...
// ../keygen/node_modules/hash-wasm/dist/index.esm.js
...
var textEncoder2 = globalObject2.TextEncoder ? new globalObject2.TextEncoder() : null;
...
function writeHexToUInt83(buf, str) {
  const size = str.length >> 1;
  for (let i = 0; i < size; i++) {
    const index = i << 1;
    buf[i] = hexCharCodesToInt3(str.charCodeAt(index), str.charCodeAt(index + 1));
  }
}
function hexStringEqualsUInt83(str, buf) {
  if (str.length !== buf.length * 2) {
    return false;
  }
  for (let i = 0; i < buf.length; i++) {
    const strIndex = i << 1;
    if (buf[i] !== hexCharCodesToInt3(str.charCodeAt(strIndex), str.charCodeAt(strIndex + 1))) {
      return false;
    }
  }
  return true;
}
var alpha3 = "a".charCodeAt(0) - 10;
var digit3 = "0".charCodeAt(0);
function getDigestHex3(tmpBuffer, input, hashLength) {
  let p = 0;
  for (let i = 0; i < hashLength; i++) {
    let nibble = input[i] >>> 4;
    tmpBuffer[p++] = nibble > 9 ? nibble + alpha3 : nibble + digit3;
    nibble = input[i] & 15;
    tmpBuffer[p++] = nibble > 9 ? nibble + alpha3 : nibble + digit3;
  }
  return String.fromCharCode.apply(null, tmpBuffer);
}
var getUInt8Buffer3 = nodeBuffer3 !== null ? (data) => {
  if (typeof data === "string") {
    const buf = nodeBuffer3.from(data, "utf8");
    return new Uint8Array(buf.buffer, buf.byteOffset, buf.length);
  }
  if (nodeBuffer3.isBuffer(data)) {
    return new Uint8Array(data.buffer, data.byteOffset, data.length);
  }
  if (ArrayBuffer.isView(data)) {
    return new Uint8Array(data.buffer, data.byteOffset, data.byteLength);
...
var base64Chars3 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
var base64Lookup3 = new Uint8Array(256);
for (let i = 0; i < base64Chars3.length; i++) {
  base64Lookup3[base64Chars3.charCodeAt(i)] = i;
}
function getDecodeBase64Length3(data) {
  let bufferLength = Math.floor(data.length * 0.75);
  const len = data.length;
  if (data[len - 1] === "=") {
    bufferLength -= 1;
...
function decodeBase643(data) {
  const bufferLength = getDecodeBase64Length3(data);
  const len = data.length;
  const bytes = new Uint8Array(bufferLength);
  let p = 0;
  for (let i = 0; i < len; i += 4) {
    const encoded1 = base64Lookup3[data.charCodeAt(i)];
    const encoded2 = base64Lookup3[data.charCodeAt(i + 1)];
    const encoded3 = base64Lookup3[data.charCodeAt(i + 2)];
    const encoded4 = base64Lookup3[data.charCodeAt(i + 3)];
    bytes[p] = encoded1 << 2 | encoded2 >> 4;
    p += 1;
    bytes[p] = (encoded2 & 15) << 4 | encoded3 >> 2;
    p += 1;
...
var MAX_HEAP3 = 16 * 1024;
var WASM_FUNC_HASH_LENGTH3 = 4;
var wasmMutex3 = new Mutex3();
var wasmModuleCache3 = /* @__PURE__ */ new Map();
function WASMInterface3(binary, hashLength) {
  return __awaiter3(this, void 0, void 0, function* () {
    let wasmInstance = null;
    let memoryView = null;
    let initialized = false;
    ...
    const loadWASMPromise = wasmMutex3.dispatch(() => __awaiter3(this, void 0, void 0, function* () {
      if (!wasmModuleCache3.has(binary.name)) {
        const asm = decodeBase643(binary.data);
        const promise = WebAssembly.compile(asm);
        wasmModuleCache3.set(binary.name, promise);
      }
      const module = yield wasmModuleCache3.get(binary.name);
      wasmInstance = yield WebAssembly.instantiate(module, {
        // env: {
        //   emscripten_memcpy_big: (dest, src, num) => {
        //     const memoryBuffer = wasmInstance.exports.memory.buffer;
    ...
    const setupInterface = () => __awaiter3(this, void 0, void 0, function* () {
      if (!wasmInstance) {
        yield loadWASMPromise;
      }
      const arrayOffset = wasmInstance.exports.Hash_GetBuffer();
      const memoryBuffer = wasmInstance.exports.memory.buffer;
      memoryView = new Uint8Array(memoryBuffer, arrayOffset, MAX_HEAP3);
    });
    const init = (bits = null) => {
      initialized = true;
      wasmInstance.exports.Hash_Init(bits);
    ...
    const updateUInt8Array = (data) => {
      let read = 0;
      while (read < data.length) {
        const chunk = data.subarray(read, read + MAX_HEAP3);
        read += chunk.length;
        memoryView.set(chunk);
        wasmInstance.exports.Hash_Update(chunk.length);
      }
    ...
    const update = (data) => {
      ...
      const Uint8Buffer = getUInt8Buffer3(data);
      updateUInt8Array(Uint8Buffer);
    };
    const digestChars = new Uint8Array(hashLength * 2);
    const digest = (outputType, padding = null) => {
      ...
      return getDigestHex3(digestChars, memoryView, hashLength);
    };
    const save = () => {
      if (!initialized) {
        throw new Error("save() can only be called after init() and before digest()");
      ...
      const memoryBuffer = wasmInstance.exports.memory.buffer;
      const internalState = new Uint8Array(memoryBuffer, stateOffset, stateLength);
      const prefixedState = new Uint8Array(WASM_FUNC_HASH_LENGTH3 + stateLength);
      writeHexToUInt83(prefixedState, binary.hash);
      prefixedState.set(internalState, WASM_FUNC_HASH_LENGTH3);
      return prefixedState;
    };
    const load = (state) => {
      if (!(state instanceof Uint8Array)) {
      ...
      const stateOffset = wasmInstance.exports.Hash_GetState();
      const stateLength = getStateSize();
      const overallLength = WASM_FUNC_HASH_LENGTH3 + stateLength;
      const memoryBuffer = wasmInstance.exports.memory.buffer;
      if (state.length !== overallLength) {
        throw new Error(`Bad state length (expected ${overallLength} bytes, got ${state.length})`);
      }
      if (!hexStringEqualsUInt83(binary.hash, state.subarray(0, WASM_FUNC_HASH_LENGTH3))) {
        throw new Error("This state was written by an incompatible hash implementation");
      }
      const internalState = state.subarray(WASM_FUNC_HASH_LENGTH3);
      new Uint8Array(memoryBuffer, stateOffset, stateLength).set(internalState);
      initialized = true;
    };
    const isDataShort = (data) => {
      if (typeof data === "string") {
        return data.length < MAX_HEAP3 / 4;
      }
      return data.byteLength < MAX_HEAP3;
    };
    let canSimplify = isDataShort;
    switch (binary.name) {
      case "argon2":
    ...
    const calculate = (data, initParam = null, digestParam = null) => {
      ...
      const buffer = getUInt8Buffer3(data);
      memoryView.set(buffer);
      wasmInstance.exports.Hash_Calculate(buffer.length, initParam, digestParam);
      return getDigestHex3(digestChars, memoryView, hashLength);
    };
...
var mutex$l3 = new Mutex3();
var mutex$k3 = new Mutex3();
var uint32View3 = new DataView(new ArrayBuffer(4));
var mutex$j3 = new Mutex3();
var name$h3 = "blake3";
var data$h3 = "";
var hash$h3 = "215d875f";
var wasmJson$h3 = {
  name: name$h3,
  data: data$h3,
  hash: hash$h3
};
var mutex$i3 = new Mutex3();
function validateBits$23(bits) {
  if (!Number.isInteger(bits) || bits < 8 || bits % 8 !== 0) {
    return new Error("Invalid variant! Valid values: 8, 16, ...");
  }
  return null;
}
function createBLAKE33(bits = 256, key = null) {
  if (validateBits$23(bits)) {
    return Promise.reject(validateBits$23(bits));
  }
  let keyBuffer = null;
  let initParam = 0;
  if (key !== null) {
    keyBuffer = getUInt8Buffer3(key);
    if (keyBuffer.length !== 32) {
      return Promise.reject(new Error("Key length must be exactly 32 bytes"));
    }
    initParam = 32;
  ...
  const outputSize = bits / 8;
  const digestParam = outputSize;
  return WASMInterface3(wasmJson$h3, outputSize).then((wasm) => {
    if (initParam === 32) {
      wasm.writeMemory(keyBuffer);
    }
    wasm.init(initParam);
...
var mutex$h3 = new Mutex3();
var mutex$g3 = new Mutex3();
var polyBuffer3 = new Uint8Array(8);
var mutex$f3 = new Mutex3();
var mutex$e3 = new Mutex3();
var mutex$d3 = new Mutex3();
var mutex$c3 = new Mutex3();
var mutex$b3 = new Mutex3();
var mutex$a3 = new Mutex3();
var mutex$93 = new Mutex3();
var mutex$83 = new Mutex3();
var mutex$73 = new Mutex3();
var mutex$63 = new Mutex3();
var mutex$53 = new Mutex3();
var seedBuffer$23 = new Uint8Array(8);
var mutex$43 = new Mutex3();
var seedBuffer$13 = new Uint8Array(8);
var mutex$33 = new Mutex3();
var seedBuffer3 = new Uint8Array(8);
var mutex$23 = new Mutex3();
var mutex$13 = new Mutex3();
...
var PkmiCryptoHandler = class _PkmiCryptoHandler {
  ...
  static async generateAddress(keyset) {
    ...
    const blake3 = await createBLAKE33();
    blake3.init();
  ...
  async loadKeysetFromObject(keysetArray) {
    _PkmiCryptoHandler.validateKeysetArray(keysetArray);
...
async function createTransaction(manifest, signerKeys) {
  ...
  const hasher = await createBLAKE33();
  hasher.init();
  hasher.update(pod);
